...
function loadStatsPage()
{
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
		{//Outer for loop iterates over each row
			for(col_counter = 4; col_counter < table.rows[row_counter].cells.length; col_counter++)
			{
...
function loadPlayersPage()
{
	...
	{
		 playerList += '<a href="#" onclick="switchPlayers('+i+')"> ' + players[i].name + '</a></br>';
	}
	document.getElementById("player_selector").innerHTML=playerList;
...
function switchPlayers(playerNum)
{
		...
		var gameCount = players[playerNum].games_played;

		var avgPyards = Math.round(pYards/gameCount);
		var avgRyards = Math.round(rYards/gameCount);
		var avgRECyards = Math.round(recYards/gameCount);
		...
		document.getElementById('g_played').innerHTML = gameCount;
